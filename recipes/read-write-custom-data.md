...
**Routes:** read via `POST /api/v2/query` (project `customData`) · write via
...
1. **Namespace your data** under `appData.<appId>` so multiple apps sharing an instance don't
   clobber each other.
2. **Read** it as an attribute of its owning instance (it's never fetched on its own).
...
5. **Delete a key** by omission: read → drop → `REPLACE` (there is no delete verb).
...
- **No delete verb** — remove keys by read → drop → `REPLACE`. APPENDing a payload *without* a key
  does not remove it.
