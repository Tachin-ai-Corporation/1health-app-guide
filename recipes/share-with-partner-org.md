...
**Routes:** `POST /api/v2/health/workflow-campaign/{id}/share` →
[agents.md](https://agents.1health.io/public/prod/api/v2/health/workflow-campaign/_id_/share/agents.md).
A single instance follows an analogous shape via `POST /api/v2/share` — verify the exact body
against the [manifest](https://agents.1health.io/public/prod/api/manifest.md) before relying on it.
...
4. **Pick the least access the partner actually needs.** `"Read"` is the common case; don't grant
   more just because it's the first value you saw in an example.
...
```ts
import { shareCampaignWithPartner, fetchCampaign } from "@/lib/api"
...
- **A single-instance (non-campaign) share is a different, less-verified endpoint** — confirm its
  exact request shape against the manifest for your platform version rather than assuming it
  mirrors the campaign-share body.
